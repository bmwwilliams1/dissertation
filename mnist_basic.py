...
def cnn():
    ...
    # # cnn.add(Dropout(p))
    # cnn.add(Convolution2D(filters=64,kernel_size=(5,5), padding = 'same',strides = (1,1),kernel_regularizer = l2(lamb)))#, kernel_initializer=RandomUniform(minval=0, maxval=0.15, seed=None)))#,kernel_regularizer = l2(lamb)))
    # cnn.add(Activation('relu'))
    # # cnn.add(Dropout(p))
    #
...
def main():
    ...
    if mode == 'TEST':
        model = keras.models.load_model('Model.01-0.9670_test9745.hdf5')
